...
public final class TimeMachineChoiceGui {
    ...
    private ModularUI createUI() {
        ...
        title.textStyle(s -> s.textColor(BUTTON_TEXT).textShadow(true).fontSize(14).adaptiveWidth(true));
    ...
    private void choose(boolean help) {
        ...
        if (connection != null) {
            connection.send(new SealedSunChoicePayload(pos, help).toVanillaServerbound());
        }
        Minecraft.getInstance().gui.setScreen(null);
    }
}
