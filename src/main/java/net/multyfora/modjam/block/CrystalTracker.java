...
public final class CrystalTracker {
    ...
    public static void notifyBlockChanged(Level level, BlockPos pos, BlockState previous, BlockState current) {
        if (!previous.is(modjam.SINGULARITY_CRYSTAL_BLOCK.get()) && !current.is(modjam.SINGULARITY_CRYSTAL_BLOCK.get())) {
            return;
        }
        ...
        synchronized (LOCK) {
            ...
            if (current.is(modjam.SINGULARITY_CRYSTAL_BLOCK.get())) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = minY; y < maxY; y++) {
                    ...
                    if (level.getBlockState(pos).is(modjam.SINGULARITY_CRYSTAL_BLOCK.get())) {
