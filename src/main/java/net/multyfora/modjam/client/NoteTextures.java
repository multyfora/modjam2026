...
public final class NoteTextures {
    ...
    public static final Identifier SQUARE = Identifier.fromNamespaceAndPath("modjam", "textures/gui/grey_sticknote.png");
    public static final Identifier SKEWED = Identifier.fromNamespaceAndPath("modjam", "textures/gui/grey_sticknote_skewed.png");
    ...
    private static Identifier create(CacheKey key) {
        ...
        try (var stream = Minecraft.getInstance().getResourceManager().getResource(source)
            ...
            var identifier = Identifier.fromNamespaceAndPath("modjam",
                    "sticknote_" + Integer.toHexString(key.color()) + (key.skewed() ? "_skewed" : ""));
            Minecraft.getInstance().getTextureManager().register(identifier, new DynamicTexture(() -> "modjam_sticknote", out));
            return identifier;
        } catch (IOException e) {
